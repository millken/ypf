...
class Database {
    ...
    public function __construct($options) {
    	...
    	$options['charset'] = empty($options['charset']) ? $this->charset : $options['charset'];
        try {
            $this->pdo = new \PDO($dsn, $options['username'], $options['password'], 
            	array(\PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES '" . $options['charset'] . "';"));
            $this->pdo->exec("SET CHARACTER SET " . $options['charset']);
            $this->pdo->setAttribute(\PDO::ATTR_ERRMODE, \PDO::ERRMODE_EXCEPTION);
            $this->pdo->query("set names " . $options['charset']);
